...
&nbsp;&nbsp;&nbsp;&nbsp;如果定义变量时没有指定初始值，则变量被默认初始化，即被赋予了默认值，而默认值到底是什么由变量类型和定义变量的位置所决定

&nbsp;&nbsp;&nbsp;&nbsp;内置类型的默认初始化值由定义的位置决定，定义于任何函数体之外的变量被初始化为0，而定义于函数体内部的内置类型变量将**不被初始化**，一个未被初始化的内置类型的值是未定义的--`undefined`，如果试图拷贝或以其他形式访问此类值将引发错误；

&nbsp;&nbsp;&nbsp;&nbsp;对于其他对象（类），各自决定其初始化对象的方式；并且，是否允许定义对象但不初始化也是由类自己定义的

&nbsp;&nbsp;&nbsp;&nbsp;建议：***初始化每一个内置类型的变量，即使这不是一个绝对必须的操作，但未初始化的变量将含有不确定的值，会造成无法预计的程序运行问题***

#### 2.2.2 变量声明和定义的关系

&nbsp;&nbsp;&nbsp;&nbsp;为了允许把程序拆分成多个逻辑部分来编写，C++语言支持 _分离式编译（separate compilation）_ 机制，该机制允许将程序分割为若干个文件，每个文件被独立编译；那么会遇到跨文件使用变量或函数的情况，例如程序中使用标准库定义的`std::cout`和`std::cin`

&nbsp;&nbsp;&nbsp;&nbsp;为了支持分离式编译，C++语言将声明与定义区分开；声明使得变量名为程序所致，一个文件如果想使用别处定义的名字必须包含对那个名字的声明，而定义负责创建与名字关联的实体；也就是说声明规定了变量的类型和名字，但定义除此之外还会申请存储空间并可能赋一个初始值

&nbsp;&nbsp;&nbsp;&nbsp;如果想声明一个变量而非定义它，在变量名前添加关键字extern，且不要显示初始化，任何包含显示初始化的声明即成为定义
...
&nbsp;&nbsp;&nbsp;&nbsp;_注：在函数体内部，如果试图初始化一个由extern关键字标记的变量，将引发错误_

&nbsp;&nbsp;&nbsp;&nbsp;***变量能且只能被定义一次，但可以被多次声明***，如果要在多个文件中使用同一个变量，就必须将声明和定义分离；此时，变量的定义必须出现在且只能出现在一个文件中，而其他用到该变量的文件必须对其进行声明，却绝对不能重复定义

&nbsp;&nbsp;&nbsp;&nbsp;C++是一种_静态类型_语言，其含义是在编译阶段检查类型，编译器负责检查数据类型以及其是否支持所参与的运算，但前提是编译器必须知道每一个对象的类型，因此需要在使用某个变量前必须声明其类型
