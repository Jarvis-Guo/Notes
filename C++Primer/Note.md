...
&nbsp;&nbsp;&nbsp;&nbsp;同一个名字出现在程序的不同位置，可能指向的是不同实体，其作用范围是大多是以花括号分隔的作用域

&nbsp;&nbsp;&nbsp;&nbsp;标识符的有效区域始于其声明语句，以声明语句所在作用域末端为结束；定义于所有花括号之外的变量具有**全局作用域（global scope）**，而定义在某个函数或某段花括号之内的变量具有**块作用域（block scope）**，在范围之外不能再访问

&nbsp;&nbsp;&nbsp;&nbsp;建议：在对象第一次被使用的地方附近定义它是一种好的选择，一方面更容易找到其定义，另一方面也会基于使用赋给它一个比较合理的初始值

&nbsp;&nbsp;&nbsp;&nbsp;**嵌套的作用域**

&nbsp;&nbsp;&nbsp;&nbsp;作用域能够彼此包含，被包含的称为内层作用域，包含其他作用域的称为外层作用域；某个作用域中声明的标识符，在其包含的所有内层作用域中都可以被访问，同时，允许在内层作用域中重新定义外层已有的名字
